...
module.exports.create = (req, res, next) => {
    ...
    db.doConnect((err, connection) => {
        if (err) {
            ...
            api.respond(res, 500, true, err, null);
        }
        var request = new Request(`
        Insert into [${database}].[dbo].[Ports] (name, price) values (${product.name}, ${product.price});`,
        function(err, rows) {
            err && api.respond(res, 500, true, err, null);
            connection.close();
            api.respond(res, 200, true, "Product Created", product);
        });
...
module.exports.read = (req, res, next) => {
    db.doConnect((err, connection) => {
        ...
        function(err, rows) {
            err && api.respond(res, 500, true, err, null);
            connection.close();
            api.respond(res, 200, true, "Products Read", data);
        });
...
module.exports.update = (req, res, next) => {
    ...
    db.doConnect((err, connection) => {
        ...
        function(err, rows) {
            err && api.respond(res, 500, true, err, null);
            connection.close();
            api.respond(res, 200, true, "Product Updated", req.params.id);
        });
        connection.execSql(request);
    });
}
...
module.exports.delete = (req, res, next) => {
    db.doConnect((err, connection) => {
        ...
        function(err, rows) {
            err && api.respond(res, 500, true, err, null);
            connection.close();
            api.respond(res, 200, true, "Product Deleted", req.params.id);
        });
        connection.execSql(request);
    });
}
